...
void thinFilmReflectance_float(float cos0, float lambda, float thickness, float n0, float n1, float n2, out float intensity) {
    const float pee=3.1415926535897932384626433832795;
    
    // compute the phase change term (constant)
    float d10 = (n1 > n0) ? 0 : pee;
    float d12 = (n1 > n2) ? 0 : pee;
    float delta = d10 + d12;
    ...
    if(sin2 > 1 || sin1 > 1) intensity = 1.0;
    else intensity = 1 - t;
}

// void thinFilmReflectance_float(float cos, float lambda, float thickness, float n0, float n1, float n2, out float Out) {
//     Out = float3(1, 0, 0);
// }
